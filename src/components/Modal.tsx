...
export default function Modal({
  ...
  return (
    <article className="w-full h-full font-Lato absolute top-0 left-0 backdrop-blur-[1px] flex flex-col justify-center items-center">
      <div className="bg-[#1F1F1F] border border-[#7a7a7a] w-11/12 aspect-2/3 flex flex-col py-4 shadow-[0_4px_4px_rgb(0_0_0/_0.5)] justify-center  items-center rounded-md">
        <section className="flex flex-col items-center relative w-full">
          <CloseMenu
            className="z-10 absolute right-3 w-10 cursor-pointer text-stone-100 hover:scale-110 transition-transform hover:text-cyan-100"
            onClick={() => {
              onClose();
            }}
          />
        ...
        <section className="flex flex-col justify-center w-full items-center flex-2 gap-3">
          <span className="text-2xl font-Lato text-stone-50">Estadisticas</span>
          <div className="flex w-full justify-between">
            {STAT_ITEMS.map(({ label, value }) => (
              <div key={label} className="flex flex-col items-center w-full">
                <span className="text-2xl mb-2 text-stone-100">{value}</span>
        ...
        <section className="w-full flex flex-col justify-center items-center flex-3">
